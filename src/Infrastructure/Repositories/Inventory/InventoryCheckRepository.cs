...
{
    public class InventoryCheckRepository(HushStoreDbContext context) : IInventoryCheckRepository
    {
        ...
        public async Task<string?> GetLastCheckCodeByDateAsync(string datePrefix)
        {
            return await _context.InventoryChecks
                .AsNoTracking()
                .IgnoreQueryFilters()
                .Where(c => c.CheckCode.StartsWith(datePrefix))
                .OrderByDescending(c => c.CheckCode)
